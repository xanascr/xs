export function generateOpt(node, types = {}, opts = {}) {
  ...
  switch (node.type) {
    ...
    case "Block":
      return `{\n${node.body
        .filter((s) => s.type !== "ExportStmt")
        .map((n) => "  " + generateOpt(n, types, opts))
        .join("\n")}\n}`;

    case "VarDecl": {
      const init = generateOpt(node.init, types, opts);
      const kw = node.kind === "const" ? "const" : node.kind === "global" ? "var" : "let";
      ...
      if (node.init?.type === "ArrayExpr" && node.init.items.every((i) => i.type === "Num")) {
        ...
        return `${kw} ${node.id}=new Float64Array([${items}])`;
      }

      return `${kw} ${node.id}=${init}`;
    }
    ...
    case "Call": {
      ...
      if (callee === "grita-ae") return `console.log(${args})`;
      if (callee === "sussurra") return `console.warn(${args})`;
      if (callee === "stalkeia") return `(await __http(${args}))`;
      if (callee === "aguenta-ai") return `(await __sleep(${args}))`;
      if (callee === "escolhe") return `__randInt(${args})`;
      if (callee === "desembola") return `JSON.parse(${args})`;
      if (callee === "bisbilhota") return `__env(${args})`;
      ...
      if (callee === "escuta") return `__createServer(${args[0]},${args[1]})`;
      if (callee === "terminamos!") return `__stopServer(${args[0]})`;
    ...
    case "TypeOf":
      return `__typeOf(${generateOpt(node.arg, types, opts)})`;

    case "InstanceOf":
      return `(${generateOpt(node.arg, types, opts)} instanceof ${generateOpt(node.cls, types, opts)})`;

    case "Spread":
      return `...${generateOpt(node.arg, types, opts)}`;
    ...
    case "ObjectExpr":
      return `{${node.props.map((p) => (p.spread ? `...${generateOpt(p.value, types, opts)}` : `${p.key}:${generateOpt(p.value, types, opts)}`)).join(",")}}`;
...
function patternToJS(pattern, valueExpr) {
  switch (pattern.type) {
    ...
    case "PatternArray":
      return `(Array.isArray(${valueExpr})&&${pattern.elements
        .map((el, i) => {
        ...
        .join("&&")})`;
    case "PatternObject":
      return `(typeof ${valueExpr}==="object"&&${valueExpr}!==null&&${pattern.props
        .map(
          (p) => `(${p.key} in ${valueExpr})&&${patternToJS(p.pattern, `${valueExpr}.${p.key}`)}`
        )
        .join("&&")})`;
    default:
      return `true`;
...
function walkTypes(node, types) {
  ...
  if (Array.isArray(node)) {
    node.forEach((n) => walkTypes(n, types));
    return;
  }
  ...
  switch (node.type) {
    ...
    case "Call": {
      if (node.callee.type === "Ident") {
        if (node.callee.name === "escolhe") return "NUM";
        if (node.callee.name === "desembola") return "OBJ";
        if (node.callee.name === "stalkeia") return "OBJ";
        if (node.callee.name === "bisbilhota") return "STR";
        if (node.callee.name === "tamanho") return "NUM";
        if (node.callee.name === "encontra") return "OBJ";
        if (node.callee.name === "juntar") return "STR";
        if (node.callee.name === "horinha") return "NUM";
      }
      return null;
    }
    case "Ternary": {
